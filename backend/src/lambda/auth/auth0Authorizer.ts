...
const jwksUrl = 'https://sls-serverless-test.eu.auth0.com/.well-known/jwks.json';
...
const getSigningKey = async (jwksUrl: string, kid: any) => {
  let res: any = await Axios.get(jwksUrl, {
    headers: {
  ...
  const signingKeys = keys.filter((key: { use: string; kty: string; kid: string; x5c: string | string[]; }) => key.use === 'sig'
    && key.kty === 'RSA'
    && key.kid
